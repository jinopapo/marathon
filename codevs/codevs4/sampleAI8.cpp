    ...
    if(u.type == ASSASSIN && u.set == 1){
      if(resourceEmenyPoint.size() != 0){
        if(u.cellNum == -1){
          ...
          u.cellNum.x = resourceEmenyPoint.v[n].x;
          u.cellNum.y = resourceEmenyPoint.v[n].y;
...
void SelectProduce(int i,int type){
  int id=-1;
  int maxdist;
  int distId=-1;
  float max=0;
  ...
  for(map<int, Unit>::iterator it = myUnits.begin(); it != myUnits.end(); it++) {
    ...
    if(u.type == type){
      ...
      if(type == WORKER_FACTORY && i!=0)scare+=5-workerNum[u.y][u.x];
      if(max < scare){
        id = u.id;
        max = scare;
      }
...
void thinkBuild() {
  ...
  for(map<int, Unit>::iterator it = myUnits.begin(); it != myUnits.end(); it++){
    ...
    if (u.type == WORKER) {
      ...
      /*if(resourceMyPoint.find(u.x,u.y) && !workerFactoryMap[u.y][u.x]){
        bool flag=false;
        for(auto i:workerFactoryPoint){
