    ...
    if (u.type == KNIGHT || u.type == FIGHTER || (u.type == ASSASSIN && u.set==2)) {
      ...
      if(ty != -1 && tx != -1){
        ...
        for(auto val:unitMap[opCastle.y][opCastle.x]){
          if(!val.member && val.type != WORKER && val.type != WORKER_FACTORY) count++;
        }
        if(unitCountMap[u.y][u.x] <= 10 && (unitMap[opCastle.y][opCastle.x].size() == 0 || count > 6)){
          u.movetoy = -1;
          u.movetox = -1;
          for(auto i:pv){
            if(u.x == i.x && u.y == i.y){
...
void thinkBuild() {
  ...
  //ワーカーの視野内に敵がいたらそのそいつが拠点を建てる
  //if(((resourceNum >= 8) || opCastle.id != -1) && warriorFactoryCount < 4) search = SearchEnemy();
  // 命令を設定
  for(map<int, Unit>::iterator it = myUnits.begin(); it != myUnits.end(); it++){
    Unit& u = it->second;
    if (u.type == CASTLE ){
    ...
    if (u.type == WORKER) {
      if(u.set == 1){
        ...
        u.set = 0;
      }else if(/*((resourceNum >= 8) || opCastle.id != -1 || search)
