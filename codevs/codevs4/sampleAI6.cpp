...
void searchResorce(Unit& u,vector<point> v){
  ...
  int y = -1;
  for(auto i:v){
    if((dist(i.y,i.x,u.y,u.x) <= 10 && workerNum[i.y][i.x] <= 5 )
       || resourceNearMyPoint.size()+resourceNearEmenyPoint.size() == 20){
      if(min > dist(i.y,i.x,u.y,u.x)){
        if(min != 0 && workerFactoryMap[i.y][i.x] == 0){
          min = dist(i.y,i.x,u.y,u.x);
          x = i.x;
          y = i.y;
        }else if(dist(i.y,i.x,u.y,u.x) == 0){
          min = dist(i.y,i.x,u.y,u.x);
          x = i.x;
          y = i.y;
        }
...
void thinkWarrior() {
  ...
  for(map<int, Unit>::iterator it = myUnits.begin(); it != myUnits.end(); it++) {
    ...
    if (u.type == KNIGHT || u.type == FIGHTER || (u.type == ASSASSIN && u.set==2)) {
      ...
      int ty = -1, tx = -1;
      if (opCastle.id != -1) {
        goToCasleRoute(resourceEmenyPoint,u,ty,tx);
        if(ty == -1) goToCasleRoute(resourceFreePoint,u,ty,tx);
        if(ty == -1){
          ty = opCastle.y;
          tx = opCastle.x;
        }
      ...
      if(warriorFactoryCount == 0){
        if(resourceNum <= 8){
          if(workerFactoryMax){
            ...
            }else if(currentResource >= 2*BUILD_COST[WORKER]){
              if (currentResource >= BUILD_COST[WORKER] && u.set == 1) {
                currentResource -= BUILD_COST[WORKER];
                u.produce = WORKER;
                u.set = 0;
